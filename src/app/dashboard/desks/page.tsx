...
import { AvailableStudent, InterimDesk, InterimShift, TermDesk, TermShift } from "@/types/deskType"
...
function ScheduleTable({ desk, isBreakTerm, selectedDesk, selectedTerm, selectedYear }: { desk: TermDesk | InterimDesk, isBreakTerm: boolean, selectedDesk: string, selectedTerm: string, selectedYear: number }) {
    ...
    const { availableStudents, fetchingAvailableStudentsError,  handleFetchAvailableStudents } = useStore();

    const handleSlotClick = async (shift: InterimShift | TermShift | undefined, dateOrDay: string, isTerm: boolean) => {
        if (shift) {
            try {
                // Using zustand store to fetch available students
                await handleFetchAvailableStudents(
                    selectedTerm.endsWith("Break"),
                    shift,
                ...
                setModalDetails({
                    ...
                    date: isTerm ? `Day: ${dateOrDay}` : `Date: ${formatDate(dateOrDay)}`,
                ...
                console.error(fetchingAvailableStudentsError, error);   
            }
        }
    };
    return (
            ...
            <p className="text-center mb-2 font-bold text-lg"> {selectedDesk.toUpperCase()} desk calendar for {selectedTerm} {selectedYear} </p>
                {isBreakTerm ? (
                    ...
                    <table className="w-full border-collapse">
                        ...
                        {/* Table Body */}
                        <tbody>
                            {desk && (desk as InterimDesk).interim_slots ? (
                                (desk as InterimDesk).interim_slots
...
export default function DesksPage() {
    ...
    const {
        ...
        // Creating desk
        creatingTermDesk,
        creatingTermDeskError,
        handleCreateTermDesk,
    ...
    const fetchData = useCallback(() => {
        handleFetchDesk(isBreak, selectedYear.toString(), selectedTerm, selectedDesk);

    }, [handleFetchDesk, isBreak, selectedYear, selectedTerm, selectedDesk]);
    ...
    const handleCreateDesk = async () => {
        ...
        try {
            await handleCreateTermDesk(payload);
    ...
    return (
        <div className="flex flex-col gap-2">
                    ...
                    <AlertDialog open={isExportDialogOpen} onOpenChange={setIsExportDialogOpen}>
                        <AlertDialogTrigger asChild>
                            <Button 
                                ...
                                disabled={exportingDesk}
                            >
                                {exportingDesk ? 'Exporting...' :(
                                    <>
                        ...
                        <AlertDialogContent>
                            ...
                            <AlertDialogFooter>
                                ...
                                <AlertDialogAction 
                                    onClick={() => handleExportDesk(isBreak, selectedYear.toString(), selectedTerm, selectedDesk)}
                                    className="text-white"
                                >
                                    {exportingDesk ? "Exporting..." : "Export"}
                                </AlertDialogAction>
                            </AlertDialogFooter>
                        </AlertDialogContent>
                    </AlertDialog>
            ...
            <div className="text-center text-muted-foreground">
                {creatingTermDeskError && <p>{creatingTermDeskError}</p>}
                {fetchingDesk ? 
                    <p>Fetching desk...</p> : (
                        <>
                            {Array.isArray(desk) && desk.length > 0 ? (
                                <>
                                    <ScheduleTable desk={desk[0] as (TermDesk | InterimDesk)} isBreakTerm={isBreak} selectedDesk={selectedDesk} selectedTerm={selectedTerm} selectedYear={selectedYear}/>
                                </> 
                            ) : (
                                <>
                                    <p>
